...
ALIGNED="inputs/studyforrest-data-aligned/sub-%s/in_bold3Tp2/sub-%s_task-%s_run-%s_bold.nii.gz"
...
WARPED="inputs/studyforrest-data-mni/sub-%s_task-%s_run-%s_bold.nii.gz"
...
do
    ...
    do
        ...
        do
            fsl5.0-applywarp \
                -i $(printf $ALIGNED $SUB $SUB $CON $RUN) \
                -r "$BRAIN" \
                -w $(printf $TEMPLATE $SUB) \
                -o $(printf $WARPED $SUB $CON $RUN)
        done
    done
done
